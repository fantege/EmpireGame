...
2. **卡牌展示**：动态创建和显示所有领袖卡牌，采用4:5宽高比设计
3. **智能布局**：根据窗口大小自动计算最优卡牌尺寸，确保所有卡牌完整显示
4. **交互功能**：为每张卡牌添加点击和悬停事件处理
5. **详细信息显示**：在底部信息面板显示选中卡牌的详细信息
6. **滚动容器**：支持多张卡牌的横向滚动浏览
7. **间距优化**：卡牌之间有20像素间距，避免重叠
...
3. **智能布局算法**：根据容器宽度和卡牌数量自动计算最优卡牌尺寸
4. **比例控制**：严格保持4:5宽高比，确保视觉一致性
5. **事件系统**：使用Godot的信号系统处理用户交互
6. **响应式布局**：使用Control节点和布局预设确保UI适配性
7. **间距管理**：通过主题常量控制卡牌间距，避免视觉拥挤
